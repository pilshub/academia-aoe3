...
import type { CardAge, Confidence, NormalizedReplay, ReplayAgeUp, ReplayHeader, ReplayShipment, ReplayTimelineEvent } from "@/data/aoe3";
...
type RawPlayer = {
  ...
  deck?: string[];
  shipments?: Array<{ time?: string; card?: string; name?: string }>;
  ageUps?: Array<{ age?: string; time?: string; politician?: string }>;
};
...
function normalizeAge(value: unknown): CardAge | "Age V" {
...
function normalizeJson(raw: Record<string, unknown>): NormalizedReplay {
...
function normalizeText(text: string): NormalizedReplay {
...
function parseReplay(text: string) {
  try {
...
export function ReplayImportLab() {
  ...
  const likelyMistakes = useMemo(() => {
    const mistakes: string[] = [];
    const ownShipments = report.shipments.filter((shipment) => shipment.player === report.header.players[0]?.name);
    const enemyMilitaryEarly = report.shipments.some((shipment) => /hussar|janissar|longbow|rush|military|soldier/i.test(shipment.card) && seconds(shipment.time) < 420);
    const greedyOwn = ownShipments.some((shipment) => /villager|cdb|coureur|company|economic|eco/i.test(shipment.card) && seconds(shipment.time) > 300);
    const lateAge3 = report.ageUps.some((age) => age.player === report.header.players[0]?.name && age.age === "Age III" && seconds(age.time) > 540);

    if (enemyMilitaryEarly && greedyOwn) mistakes.push("Patron detectado: el rival mostro shipment militar temprano y tu lado mantuvo greed economico.");
    if (lateAge3) mistakes.push("Age III supera 9:00; para Semi-FF esto suele indicar presion, macro rota o coin tardio.");
    if (ownShipments.length < 3) mistakes.push("Menos de 3 shipments detectados para tu jugador; revisa XP/TP/tesoros o export incompleto.");
